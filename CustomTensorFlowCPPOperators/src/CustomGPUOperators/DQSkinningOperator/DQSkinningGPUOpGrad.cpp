#include "DQSkinningGPUOpGrad.h"

//==============================================================================================//

REGISTER_OP("DqSkinningGpuGrad")
...
.Input("dual_quaternions: float")							// output from the forward operator	
...
DQSkinningGPUOpGrad::DQSkinningGPUOpGrad(OpKernelConstruction* context)
	:
	OpKernel(context)
{
...
void DQSkinningGPUOpGrad::setupInputOutputTensorPointers(OpKernelContext* context)
{
	//---INPUT---
	...
	//[4]
	//Grab the dual quaternions
	const Tensor& inputTensorDualQuaternions = context->input(4);
	Eigen::TensorMap<Eigen::Tensor< const float, 1, 1, Eigen::DenseIndex>, 16> inputTensorDualQuaternionsFlat = inputTensorDualQuaternions.flat<float>();
	data.d_inputDualQuaternions = inputTensorDualQuaternionsFlat.data();
...
void DQSkinningGPUOpGrad::Compute(OpKernelContext* context)
{
	try
	{
		setupInputOutputTensorPointers(context);
		computeDQSkinningGPUOpGradGPU(data);
	}
	catch (std::exception e)
	{
		std::cerr << "Compute DQ skinning grad error!" << std::endl;
	}
}

//==============================================================================================//

REGISTER_KERNEL_BUILDER(Name("DqSkinningGpuGrad").Device(DEVICE_GPU), DQSkinningGPUOpGrad);
