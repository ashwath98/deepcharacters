...
from Settings.SettingsTMap import Settings
import time
TEX_WIDTH=1024
TEX_HEIGHT=1024
NUM_CAMERAS=4
...
class TextureMapping(tf.Module):
  ...
  def setup_renderer_inputs(self):
    ...
    self.inputTexture = tf.tile(inputTexture, (settings.numberOfBatches, 1, 1, 1))
  ...
  @tf.function
  def __call__(self, inputVertexPositions,frame_number,images_masked):
    ############################ INPUT ########################################################
    inputVertexPositions = tf.reshape(inputVertexPositions,[1, inputVertexPositions.shape[0], 3])
...
start_frame = int(sys.argv[1])#start frame
total_frames = int(sys.argv[2])#total number of frames you want textures for
output_folder=sys.argv[3]#output_folder

settings=Settings(None)

my_model = TextureMapping(settings=settings,name="TMapTest")
inputVertexPositions=CreateMeshTensor.createMeshSequenceTensor(settings.meshes_file, start_frame, total_frames)
print(inputVertexPositions.shape)
for k in range(total_frames):
    frameString=tf.strings.as_string([start_frame+k])
    #inputVertexPositions_c=inputVertexPositions[k]
    images,fgmasks=read_images_tf(settings,frameString)
    images_masked=images*fgmasks
    time1=time.time()
    
    inputVertexPositions_c=inputVertexPositions[k]
    final_texture=my_model(inputVertexPositions_c,frameString,images_masked)
    time2=time.time()
    save_color_image(np.float32(final_texture.numpy()),os.path.join(output_folder,'frame_'+str(start_frame+k)+'_median'+'.jpg'))
